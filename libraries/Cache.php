...
class Cache
{
  ...
  private function _call($property, $method, $arguments = array(), $expires = NULL)
  {
    ...
    $hash = dohash($method.serialize($arguments), 'sha1');
    
    // Split cache files into folders based on hash fragments
    $cache_path = $property.DIRECTORY_SEPARATOR.substr($hash,0,2).DIRECTORY_SEPARATOR.substr($hash,2,2);
  ...
  function write($contents = NULL, $filename = NULL, $expires = NULL, $dependencies = array())
  {
    ...
    if (count($subdirs) > 1) : while (count($subdirs) > 1)
    {
      array_pop($subdirs);
